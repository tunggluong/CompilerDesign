...
Token* readIdentKeyword(void) {
  ...
  while (currentChar != EOF &&
    ...
    if (i < MAX_IDENT_LEN) token->string[i++] = currentChar;
    else tooLong = 1;
  ...
  token->tokenType = (kw != TK_NONE) ? kw : TK_IDENT;
...
Token* getToken(void) {
  ...
  if (currentChar == EOF) return makeToken(TK_EOF, lineNo, colNo);
