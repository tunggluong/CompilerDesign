  ...
  while (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_TIMES) {
      ...
      if (charCodes[currentChar] == CHAR_RPAR) {
        // kết thúc comment -> đọc tiếp sau ')'
        readChar();
        return;
      }
      // nếu không phải ')', tiếp tục đọc
    } else {
      readChar();
    }
  }
...
/* Đọc định danh / từ khoá: bắt đầu bằng LETTER, sau đó LETTER/DIGIT */
Token* readIdentKeyword(void) {
  ...
  while (currentChar != EOF &&
    ...
    if (i < MAX_IDENT_LEN) {
      ...
      // vượt quá độ dài cho phép, vẫn đọc nhưng không lưu thêm
      tooLong = 1;
    }
    readChar();
  }
  ...
  if (kw != TK_NONE)
    token->tokenType = kw;      // là từ khoá
  else
    token->tokenType = TK_IDENT; // là định danh
  return token;
}
...
/* Đọc hằng ký tự dạng 'a' */
Token* readConstChar(void) {
  Token *token = makeToken(TK_CHAR, lineNo, colNo);

  // currentChar đang là dấu '
  readChar();
  if (currentChar == EOF) {
    token->tokenType = TK_NONE;
...
Token* getToken(void) {
  ...
  case CHAR_EXCLAIMATION:   // '!' -> có thể là '!='
    ln = lineNo; cn = colNo;
    readChar();
    if (currentChar != EOF && charCodes[currentChar] == CHAR_EQ) {
      readChar();
