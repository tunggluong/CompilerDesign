...
void compileFuncDecl(void) {
  ...
  // declare the function object (needed for recursion)
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // unsigned constant can be an integer constant identifier
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    ...
    if (obj == NULL) {
      ...
      // must be integer constant in +/- context
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    ...
    if (obj == NULL) type = makeIntType();
    else type = duplicateType(obj->typeAttrs->actualType);
    break;
  default:
...
void compileLValue(void) {
  ...
  // only variable/parameter may have indexes; function-result assignment doesn't
  if (lookAhead->tokenType == SB_LSEL) {
    if (var != NULL && (var->kind == OBJ_VARIABLE || var->kind == OBJ_PARAMETER)) {
      compileIndexes();
    } else {
...
void compileForSt(void) {
  ...
  // check if the identifier is a variable/parameter that can be assigned
